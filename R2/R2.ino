...
// Catapult pins
int cdcv_pins[] = {100, 100};

bool cdcv = 1;

// Ball picking pins
int bdcv_pins[] = {100, 100};
int bd_pin = 3, bp_pin = 4;

const int bspd = 100, btime = 1000;
bool bdir = 1;
...
void loop() {
  ...
  if (PS3.PS3Connected || PS3.PS3NavigationConnected) {
    ...
    if (PS3.getButtonPress(L1)) {
      if (isLP) {
        Serial.println("L1 is being pressed");
        motorWrite('l', lspd, 1);
      }
      else {
        motorWrite('b', bspd, 1);
  ...
  switch (opt) {
    ...
    case 'l': {
      if(spd > 0) {
         ...
         Serial.print("Motor is working with ");
         Serial.print(spd);
         Serial.print(" speed and dir: "); 
         Serial.println(dir);
      }
